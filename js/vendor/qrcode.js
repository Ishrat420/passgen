 ...
 *
 * Derived from the original work by Kazuhiko Arase and the qrcode-terminal
 * project. Packaged for browser use so the encoder can run fully offline.
 *
...
(function (global) {
  ...
  QRCodeModel.createBytes = function (buffer, rsBlocks) {
    ...
    for (let r = 0; r < rsBlocks.length; r++) {
      ...
      for (let i = 0; i < dcdata[r].length; i++) {
        dcdata[r][i] = buffer.buffer[i + offset] & 0xff;
      }
      offset += dcCount;
  ...
  function resolveCanvas(element) {
    if (typeof element === 'string') {
      return global.document ? global.document.getElementById(element) : null;
    }
    return element;
  }
  ...
  function toCanvas(canvas, text, options, callback) {
    ...
    try {
      ...
      const level = (opts.errorCorrectionLevel || 'M').toUpperCase();
      ...
      if (resolvedCanvas.style) {
        ...
        resolvedCanvas.style.maxWidth = '100%';
        resolvedCanvas.style.maxHeight = '100%';
  ...
  const QRCode = {
    toCanvas,
    create(text, options = {}) {
      const level = (options.errorCorrectionLevel || 'M').toUpperCase();
      return createQRCode(String(text), QRErrorCorrectLevel[level] ?? QRErrorCorrectLevel.M);
    },
    QRErrorCorrectLevel
  };
